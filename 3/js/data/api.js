...
function matchHomeTeamData(homeData){
   let playerList = '';
   let printPlayerList = '';
   let coachName = '-';
...
function matchAwayTeamData(awayData){
   let playerList = '';
   let printPlayerList = '';
   let coachName = '-';
...
function standingList(){
   ...
   if('caches' in window){
      caches.match(`${base_url}/competitions/2000/standings?standingType=TOTAL`)
      .then(response => {
         if(response){
            parseJson(response)
            .then(datas => {
